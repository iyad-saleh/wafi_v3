...
from  .models import Coin, Country,  AirPort,City, Coin_log

@admin.register(Coin)
class CoinAdmin(ImportExportModelAdmin):
    list_display  = [f.name for f in Coin._meta.fields]
    class Meta:
        model = Coin


@admin.register(Coin_log)
class Coin_logFormAdmin(ImportExportModelAdmin):
    list_display  = [f.name for f in Coin_log._meta.fields]
    class Meta:
        model = Coin_log
